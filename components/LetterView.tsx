...
export const LetterView: React.FC<LetterViewProps> = ({
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-[90vh] justify-center relative py-20 animate-fade-in">
      ...
      {/* 3D Envelope Layer */}
      <div
        className={`transition-all duration-1000 absolute inset-0 flex items-center justify-center z-10
      ...
      >
        <div
          className={`relative ${design.letter.paperBg} ${design.letter.borderColor} border-[8px] md:border-[16px] p-8 md:p-24 shadow-[0_50px_100px_rgba(0,0,0,0.3)] md:shadow-[0_100px_200px_rgba(0,0,0,0.4)] mb-12 flex flex-col min-h-[600px] md:min-h-[900px] overflow-hidden`}
        >
          {/* Deckled Edge */}
          <div
          ...
          {/* Inner Foil Frame */}
          <div
            className="absolute inset-6 md:inset-10 border-2 border-transparent pointer-events-none"
            style={{
              borderImage:
          ...
          <div className="relative z-10 flex-grow flex flex-col items-center">
            ...
            <div
              className={`w-full max-w-3xl ${design.letter.fontFamily} ${design.letter.textColor} text-xl md:text-5xl leading-[2.2] md:leading-[2.6] tracking-wide whitespace-pre-wrap text-center drop-shadow-[0_1px_1px_rgba(0,0,0,0.1)]`}
            >
              {displayedText}
        ...
        {/* Actions */}
        <div
          className={`flex flex-wrap justify-center gap-6 transition-all duration-1000 delay-1000 ${stage === 'reading' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          <Button variant="secondary" onClick={onEdit} className="rounded-full px-10 border-stone-200 shadow-xl bg-white">
            Modify Design
          </Button>
          <Button onClick={onDispatch} className="rounded-full px-10 bg-stone-900 text-white shadow-2xl hover:bg-black">
            Seal & Dispatch
          </Button>
